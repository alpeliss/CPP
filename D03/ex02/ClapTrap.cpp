...
void ClapTrap::takeDamage(unsigned int amount){
    if (amount < 0){
        std::cout <<  this->_type <<  " can't be healed by an attack" << std::endl;
        return ;
    }
    if (this->_hp <= 0){
        std::cout << this->_type << " " << this->_name;
    ...
    if (this->_hp < amount)
        ...
        this->_hp -= amount;
    std::cout << this->_type << " " << this->_name << " get hit and loose ";
...
void ClapTrap::beRepaired(unsigned int amount){
    if (amount < 0){
        std::cout << this->_type << " can't harm itself" << std::endl;
        return ;
    }
    if (this->_hp <= 0){
        std::cout << this->_type << " " << this->_name;
    ...
    if (this->_hp > this->_vie)
        this->_hp = this->_vie;
    std::cout << this->_type << " " << this->_name << " heals himself of ";
    std::cout << amount << " hp: " << this->_hp << " left!" << std::endl;
    if (this->_hp == this->_vie)
        std::cout << this->_type << " is now perfectly fine =)" << std::endl;
    this->_ep--;
    return ;
}
